...
int main(int argc, char const *argv[]) {
    ...
    int current_score;
    int tabu_size = (x_edt.size() * x_edt[0].size() * x_edt[0][0].size())/4;
    if (argc == 4){
    ...
    for (int i = 0; i < global_solution.size(); i++){
        ...
        if (b_e[i] > test){
            global_best_score = global_best_score - (b_e[i] - test)*10000;
        }
        test = 0;
    }
    test = 0;
        ...
        for (int j = 0;j < global_solution[i].size(); j++){
            if (accumulate(global_solution[i][j].begin(), global_solution[i][j].end(), 0) == 0){
                ...
                if (test < o_e[i] && test != 0){
                    global_best_score = global_best_score - (o_e[i] - test)*10000;
                }
                test = 0;
            }
        }
        ...
        for (int j = 0;j < global_solution[i].size(); j++){
            if (accumulate(global_solution[i][j].begin(), global_solution[i][j].end(), 0) == 1){
                ...
                if (test < f_e[i] && test != 0){
                    global_best_score = global_best_score - (f_e[i] - test)*10000;
                }
                test = 0;
            }
        }
